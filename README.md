...
- **`cgc_app`** — real Row-Level-Security-enforced role for tenant-scoped writes/reads (PoD's 3 tables, the 4 tenant-scoped `cgc_guard` tables).

  **`cgc_tco.audit_trail` is deliberately admin-pool-only, resolved 2026-09-14.** This table previously had a `tenant_scope` RLS policy (`tenant_id = current_setting(...)`) left over from `app/Core/db/database.py`'s provisioning code, contradicting this file's own older claim that RLS was "deliberately excluded" here. Live-DB verification (2026-09-14) confirmed the policy was not just inert but actively misleading: `tenant_id` and `app_source` are **not** the same identifier space and are **not** kept in sync — for `app_source='ledgiproof'`, `tenant_id` holds dozens of distinct ad-hoc testing/debug strings (`cgc-concurrency-test-001`, `cgc-verify-phase2-sqli`, etc.), not a real per-tenant identifier, while the real customer-facing query (`/tenants/my-apps/{app_source}/decisions`) has always filtered by the `app_source` column in application code. Activating the policy would not have scoped data correctly even if it had been wired up. The stale policy was dropped (`DROP POLICY tenant_scope ON cgc_tco.audit_trail`) rather than fixed — this table is one global sequential hash chain, not per-tenant, and its block-numbering logic needs unrestricted visibility; app-code `app_source` filtering (as already implemented) is the correct and only isolation mechanism here, not RLS. See `docs/INTEGRATION_GUIDE.md` §7.2 for what this means for an external integrator.

  **Separately found and fixed the same day**: six secondary-feature tables (`cgc_launch_errors`, `cgc_launch_snapshot`, `cgc_launch_checklist_items`, `cgc_tenant_webhooks`, `cgc_saml_connections`, `cgc_calibration_changelog`) had full `SELECT/INSERT/UPDATE/DELETE/TRUNCATE` granted to Supabase's `anon` and `authenticated` PostgREST roles with zero RLS policies — Supabase's default grants on table creation, never revoked, and never actually needed since this app only ever talks to Postgres directly via `psycopg2` (`get_connection()`), never through PostgREST/the anon key. `cgc_saml_connections` (enterprise SSO config) and `cgc_tenant_webhooks` (webhook URLs) being world-writable via an unauthenticated public key was the most severe of the six. Fixed: RLS enabled + `anon`/`authenticated` grants revoked on all six; verified via Supabase's own security advisor (`rls_disabled_in_public` ERROR count: 6 → 0) and confirmed the app itself is unaffected (it never used those roles).
