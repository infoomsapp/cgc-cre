...
- **`cgc_app`** — real Row-Level-Security-enforced role for tenant-scoped writes/reads (PoD's 3 tables, the 4 tenant-scoped `cgc_guard` tables).
  ...
  **`cgc_tco.audit_trail` — documentation/code drift, found 2026-09-14, not yet resolved.** A `tenant_scope` RLS policy (`tenant_id = current_setting('cgc.current_tenant_id')`) and a `GRANT` to `cgc_app` DO exist for this table (`app/Core/db/database.py`, provisioning block) — this file previously claimed the opposite ("deliberately excluded"), which was inaccurate. In practice the policy is currently inert: every real read/write to `audit_trail` (`app/modules/tco/tcomodule.py`) goes through the unrestricted `postgres` admin connection, never `get_scoped_connection()`, and the customer-facing query (`/tenants/my-apps/{app_source}/decisions`) is scoped by an `app_source` column filter in application code, not by the `tenant_id` column the RLS policy checks. Those may not even be the same identifier space — **needs live-DB verification** (confirm whether `audit_trail` has both `tenant_id` and `app_source` columns, and whether they're kept in sync) before deciding whether to (a) actually route audit_trail reads through `get_scoped_connection()` so the existing policy does real work, or (b) drop the now-pointless policy and keep documenting this as consciously admin-pool-only, matching the original rationale about the global hash chain needing unrestricted visibility for block-numbering. Tracked as open, not fixed.
