...
- **Only one SSO provider (Google OIDC), no SAML.** "Continue with Google" covers social/Workspace login; Microsoft/Azure AD, Okta, or any SAML-based enterprise SSO are not supported — SAML is notoriously hard to implement safely from scratch and was deliberately not attempted here.
