...
These are the real, currently-unmitigated gaps as of 2026-09-14 — kept accurate deliberately, since this file is the first thing an external auditor or acquirer would check against the actual code:
...
- **8 RLS policies re-evaluate `current_setting()` per row instead of once per query** (Supabase advisor `auth_rls_initplan`, WARN, performance not security — every `tenant_scope` policy across `cgc_pod`/`cgc_guard`/`cgc_tco`) — the fix is wrapping each call as `(select current_setting(...))` in the `USING` clause, not yet done. A real query-plan optimization at scale, not a correctness or security issue.
