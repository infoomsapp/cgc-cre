...
- **13 database functions have a mutable `search_path`** (Supabase advisor `function_search_path_mutable`, WARN) and `public.cgc_pod_chain_immutable()` is directly callable by `anon`/`authenticated` via PostgREST RPC despite being `SECURITY DEFINER` — found 2026-09-14 alongside the public-schema grant sweep below, not yet fixed (different remediation: `SET search_path` per function / `REVOKE EXECUTE`, not a grant/policy fix).
